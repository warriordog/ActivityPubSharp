...
Provides a [pre-configured serializer](JsonLdSerializer.cs) to convert between models and JSON.
...
* [`ActivityPub.Types.AS.Collection`](Collection.) - Collection / page types.
* [`ActivityPub.Types.AS.Extended.Object`](Object.) - Extended Object types. These all derive from [`ASObject`](ASObject.cs).
* [`ActivityPub.Types.AS.Extended.Link`](Link.) - Extended Link types. These all derive from [`ASLink`](ASLink.cs).
* [`ActivityPub.Types.AS.Extended.Actor`](Actor.) - Extended Actor types. These all derive from [`APActor`](APActor.cs).
* [`ActivityPub.Types.AS.Extended.Activity`](Activity.) - Extended Activity types. These all derive from [`ASActivity`](ASActivity.cs) or one of the synthetic types described below.
...
* [`APActor`](APActor.cs) - base type for Actor objects. Defined object types may extend this directly, and any other compatible object (includes Inbox and Outbox) will be "promoted" to include this as a standalone.
...
2. Try all entities that implement [`INamelessEntity`](INamelessEntity.cs).
3. Try all entities that implement [`IAnonymousEntity`](IAnonymousEntity.cs).
4. Try all configured [`IAnonymousEntitySelector`](IAnonymousEntitySelector.cs) instances.
...
* [`ASUri`](ASUri.cs) - (will) implement the namespaced IRI concept used in JSON-LD. For now, just a wrapper around `Uri`.
* [`JsonLDContext`](JsonLDContext.cs) - provides a consolidated view of the JSON-LD context associated with an object.
* [`JsonLDContextObject`](JsonLDContextObject.cs) - encapsulates the `object | IRI` typing used in the JSON-LD context. Does not parse or enforce IRI correctness - its just a `string`.
* [`JsonLDTerm`](JsonLDTerm.cs) - encapsulates the `object | IRI` typing used in JSON-LD term definitions. Does not parse or enforce IRI correctness - its just a `string`.
* [`Linkable<T>`](Linkable.cs) - encapsulates the `Range: Link | T` construction used in ActivityStreams.
* [`LinkableList<T>`](LinkableList.cs) - encapsulates the `Range: (Link | T)[]` construction used in ActivityStreams, as well as the extra cursed `Link | T | (Link | T)[]` variant.
* [`NaturalLanguageString`](NaturalLanguageString.cs) - supports language-tag mapped strings. The current implementation is subpar and will be replaced in [#12](https://github.com/warriordog/ActivityPubSharp/issues/12).

## Collections

ActivityStreams Collection types are implemented in the [`ActivityPub.Types.AS.Collection` namespace](Collection.).
...
At startup, the module scans all loaded assemblies and registers any type deriving from [ASType](ASType.cs) and implementing [IASModel](IASModel.cs).
Registered extension classes will be automatically constructed by the [polymorphic converter](TypeMapConverter.cs).
...
* Only a minimal subset of JSON-LD is supported. For full compatibility, it is necessary to override the default [JsonLdSerializer](JsonLdSerializer.cs) and pre-process the incoming JSON.

## Available Options

| Options Class                                                    | Configuration Path          | Description                                                                |
|------------------------------------------------------------------|-----------------------------|----------------------------------------------------------------------------|
| [JsonLdSerializerOptions](JsonLdSerializerOptions.cs) | N/A - not mapped by default | Options for the default [JsonLdSerializer](JsonLdSerializer.cs) |
| [ConversionOptions](ConversionOptions.cs)   | N/A - not mapped by default | General JSON / JSON-LD conversion options                                  |
